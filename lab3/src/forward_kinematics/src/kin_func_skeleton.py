...
def rotation_3d(omega, theta):
    ...
    if not omega.shape == (3,):
        ...
        rot1 = (omega_hat/omega_norm)*np.sin(omega_norm*theta)
        rot2 = (1 - np.cos(omega_norm*theta))*np.dot(omega_hat, omega_hat)/omega_norm**2
        rot = np.identity(3) + rot1 + rot2
        return rot
...
def homog_2d(xi, theta):
    ...
    if not xi.shape == (3,):
        ...
        p = (np.identity(2) - R).dot(np.array([[0, -1], [1, 0]])).dot(np.array([[xi[0]/xi[2]], [xi[1]/xi[2]]]))
...
def homog_3d(xi, theta):
    ...
    if not xi.shape == (6,):
        ...
        v = np.array([xi[:3]]).T
        omega = np.array([xi[3:]]).T
        omega_norm = np.linalg.norm(xi[3:])
        g1 = rotation_3d(xi[3:], theta)
        g2 = ((np.identity(3) - g1).dot(skew_3d(xi[3:]).dot(v)) + omega.dot(omega.T).dot(v)*theta)/omega_norm**2
        g = np.vstack((np.hstack((g1, g2)), [0, 0, 0, 1]))
        return g
